...
>
> 本记录只记录本次实现所在环境的实际结果；PostgreSQL gate 因环境限制单独列出，其他已执行的浏览器 E2E 按实际结果记录。
...
- ZNCK-I fixture 保持 raw 数据语义，覆盖 `8166`、FC16 `8120=0`、50ms 策略、FC03 `8121..8137`、边沿抑制、恢复再触发、异常和请求日志。
...
| 模拟器 | `uv run python -m unittest discover -s tests -v` | 18 项通过 |
...
| ZNCK-I 真实动态 runtime | `task e2e:znck-i` | 通过（RTU + TCP；FC16 → delay → FC03；raw event；同码抑制） |

## 环境限制

- PostgreSQL integration 未能执行：当前 WSL 的 Docker Desktop integration / daemon 不可用，`docker version` 返回该 WSL 发行版未发现 Docker，因此无法启动隔离 PostgreSQL；不能将该 gate 记为通过。
