...
func TestAcquisitionDTOUsesTransportNeutralFields(t *testing.T) {
	...
	if err := json.Unmarshal([]byte(`{"name":"网络设备","deviceType":"GENERIC","channelId":3,"unitId":0,"scriptId":9,"networkEndpoint":{"host":"[2001:db8::1]","port":1502},"pollIntervalMs":1000,"failureThreshold":3,"enabled":1}`), &device); err != nil {
		t.Fatal(err)
	}
	...
	if deviceInput.ScriptID == nil || *deviceInput.ScriptID != 9 {
		t.Fatalf("device input script ID = %v, want 9", deviceInput.ScriptID)
	}
}
