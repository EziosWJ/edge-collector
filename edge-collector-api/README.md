# edge-collector 本地开发与调试

`edge-collector-api` 是基于 Go、Gin、GORM 的 REST API，PostgreSQL 为默认数据库，SQLite 也可作为正式的单实例生产数据库。数据库结构与内置数据由独立的 Goose migration 管理；API 启动时不会执行 migration 或 GORM `AutoMigrate()`。
...
```yaml
database:
  url: postgres://127.0.0.1:54329/edge_collector?sslmode=disable
...
任务会自动加载 `configs/config.sqlite.yaml`；如需更换数据库文件位置，直接修改该文件的 `database.url`。首次使用时请确保 `edge-collector-api/.data` 目录存在，并准备好 JWT 配置。
...
```zsh
cd ..
task db:backup -- --source /var/lib/edge-collector-api/data/edge-collector.db \
  --destination /var/lib/edge-collector-api/backups/edge-collector-$(date +%Y%m%d%H%M%S).db \
  --verify
```
...
```zsh
cp /var/lib/edge-collector-api/backups/edge-collector-verified.db /var/lib/edge-collector-api/data/edge-collector-restored.db
APP_ENV=prod APP_DATABASE__URL=/var/lib/edge-collector-api/data/edge-collector-restored.db go run ./cmd/migrate up --kind all
APP_ENV=prod APP_DATABASE__URL=/var/lib/edge-collector-api/data/edge-collector-restored.db go run ./cmd/api
```
...
```zsh
docker volume rm edge_collector_postgres_data
```
...
不使用 VS Code 时，可在 `edge-collector-api` 目录执行相同命令：
...
```zsh
cd edge-collector-api
GOCACHE=/tmp/edge-collector-build go test ./...
```
...
```zsh
cd edge-collector-api
GOCACHE=/tmp/edge-collector-build go test -tags=integration ./integration
```
...
```zsh
cd edge-collector-api
GOCACHE=/tmp/edge-collector-build go test -tags=integration ./integration -run SQLite
```
