...
**部署实例**：一次运行中的 Edge Collector 服务及其所管理的通信通道、设备和运行状态边界；当前 Dashboard 的观察范围是单个部署实例。_Avoid_：项目（当它被用来表示运行边界时）。

**运维概览**：面向运维人员、用于只读判断单个部署实例当前是否正常并定位异常入口的首页视图；它不等同于聚合用户、日志和通知待办的通用管理工作台。_Avoid_：工作台（当它被用来表示泛后台首页时）。

**健康分域**：运维概览中彼此独立的运行观察边界；当前包括采集运行、设备通信、MQTT 上行和服务/数据库，不把这些边界压缩成一个无法解释来源的单一状态。_Avoid_：系统健康（当它被用来表示所有边界的单一布尔结论时）。

**状态摘要**：对一个健康分域内当前对象状态的数量分布；它描述当前观察结果，不代表历史可用率、吞吐量或业务指标。

**运维关注项**：当前状态或状态数据获取问题导致运维人员需要进一步查看的对象或分域；它可以来自通道/设备的部分失败或离线、MQTT 运行异常、服务不可用或数据过期，但不等同于正式业务告警。_Avoid_：告警（当它只表示运维状态异常时）。

**层级关注项**：按照服务/数据库、MQTT、通信通道、设备和脚本运行的边界聚合后的运维关注项；上层通道异常展示受影响设备数量，不在同一层重复展开由该通道直接导致的设备异常。
...
- 采集底座已覆盖四种 Modbus transport 的固定 FC03/FC04 raw `registerBlocks` 轮询；ADR-0016 的 Starlark draft/Validate/Publish/Rollback、设备绑定、受控 `after_poll`、state/event overlay 和脚本运行观察也已实现。ADR-0017 的 MQTT Client、latest-state raw/status、reliable outbox、command journal、`command(ctx,name,args)`、远程控制和管理页面已按 #40～#47 实现；真实 PostgreSQL/Broker 矩阵仍需在 Docker 可用环境完成。业务工程量解析、正式 telemetry/alarm 和业务告警持久化仍属于后续阶段。
...
**动态事件**：脚本通过 `ctx.emit_event` 产生的有界、可去重、JSON-compatible 运行时输出。当前已实现阶段不是业务告警历史；ADR-0017 将成功提交的动态事件投影到 reliable MQTT event，但仍不承担正式业务告警上报。

### MQTT 上下行（ADR-0017 已实现）
